...
const deploy = async () => {
    ...
    const feeApproverContract = await new web3.eth.Contract(FeeApprover.abi)
        ...
        .send({ gas: "7000000", from: mainAccount });
    ...
    await nerdVaultContract.methods.initialize(nerdContract.options.address, getConfig.getDevAddress()).send({ gas: "7000000", from: mainAccount });
    ...
    await farmETHRouterContract.methods.initialize(nerdContract.options.address).send({from: mainAccount});
